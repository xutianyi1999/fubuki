...
- **[Fubukidaze](https://github.com/darkokoa/Fubukidaze)** — Fubuki 的社区 Android 客户端。
...
所有选项均在 JSON 配置文件中，通过 `fubuki server daemon -c <path>` 或 `fubuki node daemon -c <path>` 传入。
...
```bash
fubuki server daemon -c /path/to/server.json   # 启动服务器（别名：start）
fubuki server info                             # 状态 TUI（别名：status，默认 API: 127.0.0.1:3031）
fubuki server restart                          # 重启服务器

fubuki node daemon -c /path/to/node.json       # 启动节点（别名：start）
fubuki node info                               # 状态 TUI（别名：status，默认 API: 127.0.0.1:3030）
fubuki node restart                            # 重启节点

fubuki update                                  # 自更新至最新发布版本
```

`info` 和 `restart` 均支持 `-a <ADDR>` / `--api <ADDR>` 指定非默认 API 地址。
...
- **TUI**（终端界面）：运行 `fubuki node info` 或 `fubuki server info` 打开状态 TUI。使用 `-a`/`--api` 可指定非默认 API 地址。
