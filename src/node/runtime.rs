...
use crate::common::hook::Hooks;
use crate::common::net::protocol::{
    AllocateError, GroupContent, HeartbeatType, NetProtocol, PeerStatus, Register,
    RegisterError, Seq, TcpMsg, UdpMsg, UdpSocketErr, VirtualAddr, SERVER_VIRTUAL_ADDR,
    TCP_BUFF_SIZE, TCP_MSG_HEADER_LEN, UDP_BUFF_SIZE, UDP_MSG_HEADER_LEN,
};
use crate::common::net::{
    HeartbeatCache, SocketExt, UdpStatus,
};
...
use crate::{common, routing_table, Cipher, Context, NodeConfigFinalize, TargetGroupFinalize};

use super::packet::PacketSender;
use super::routing::lookup_host;
use super::types::{
    AtomicAddr, AtomicCidr, Direction, ExtendedNode, Interface, InterfaceInfo, NodeList,
    NodeListOps, RoutingTableEnum,
};
async fn tun_handler<T, K, InterRT, ExternRT>(
    ...
    snat: Option<Arc<super::cross_nat::SNat>>
) -> Result<()>
    where
        ...
        T: TunDevice + Send + Sync + 'static,
{
    ...
    let snat = if config.cross_nat && allowed_ips_exists {
        let snat = super::cross_nat::SNat::create(
            rt.clone(), 
